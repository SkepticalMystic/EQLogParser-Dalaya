...
namespace EQLogParser
{
  class PlayerRegistry : ILifecycle
  {
    ...
    {
      // Dalaya named pets ("Bonaparte ", "Fireballs ") arrive with a trailing space.
      // Normalise here so the registry stores the canonical name; the trailing-space
      // signal is consumed at the parser detection sites before this is called.
      pet = pet?.Trim();
      var needEvent = false;
    ...
    {
      // Dalaya named pets carry a trailing space; normalise before storing.
      name = name?.Trim();
      if (string.IsNullOrEmpty(name))
        return;
