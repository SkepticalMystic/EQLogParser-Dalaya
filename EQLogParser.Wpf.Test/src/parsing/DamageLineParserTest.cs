...
namespace EQLogParser.Wpf.Test
{
  ...
  public class DamageLineParserTest
  {
    ...
    private DamageLineParser? _parser;
...
#pragma warning restore CS8603 // Possible null reference return.
      ...
      _parser = new DamageLineParser(_mockDataManager.Object, PlayerRegistry.Instance)
      {
        FightManager = _mockFightManager.Object
      };
    }
    ...
    private DamageRecord ParseAction(string action)
    {
      return _parser!.ParseLine(action);
    }
    ...
    [TestMethod]
    [Ignore("Upstream's 2.3.51 branch for 'X was hit by non-melee' is replaced in Dalaya by the _pendingDs mechanism; the line is stashed pending the next melee hit instead of producing a damage record.")]
    ...
    [TestMethod]
    [Ignore("Live EQ format 'from <spell> by <attacker>'. Dalaya's reversed format is 'from <attacker> by <spell>' and the parser pre-swaps unconditionally; this test would require the live format which Dalaya does not support.")]
    ...
    [TestMethod]
    [Ignore("Live EQ format; see TestTaken_DamageFromSpell.")]
    ...
    [TestMethod]
    [Ignore("Live EQ format with empty attacker after 'by'; Dalaya's pre-swap leaves spell empty so no record is produced.")]
